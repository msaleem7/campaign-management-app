class NovicesController < ApplicationController
    ...
    def update
        if @novice.update(novice_params)
            ...
            flash[:success] = "Novice was successfully updated"
            redirect_to @novice
        else
          flash[:error] = "Something went wrong"
          render 'edit'
    ...
    def destroy
        ...
        authorize @novice
        if @novice.destroy
            flash[:success] = 'Novice was successfully deleted.'
    ...
    def novice_params
        current_user.type=="Novice" ? params.require(:novice).permit(:username,:email,:password,:type) : params.require(:novice).permit(:username,:email,:status)
